...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (loadedSettings.floatingWindowPosition) {
    const floatingPositionLeftAndTop =
      loadedSettings.floatingWindowPosition.split(" + ");
  ...
  () => {
    ...
    nav.nextElementSibling
      ...
      .addEventListener("click", () => {
        settingWindow.style.transform = "translateX(9999px)";
      });
  },
  true,
);
...
function floatingWindowOnActive() {
  ...
  let createOptionsFromAyahNames = Array.from(verseInfo)
    .map((el) => {
  ...
  // Run only once
  let lengthOfAyah =
    verseInfo[0].nextElementSibling.querySelectorAll("div .ayah").length;
  ...
  ayahName.addEventListener("change", (e) => {
    if (verseInfo[ayahName.selectedIndex] !== undefined) {
      let lengthOfAyah =
        verseInfo[ayahName.selectedIndex].nextElementSibling.querySelectorAll(
          "div .ayah",
        ).length;
      ayahNumber.placeholder = lengthOfAyah;
...
goBtn.addEventListener("click", (e) => {
  e.stopPropagation();
  currentVerseInfo[currentVerseIndex].nextElementSibling
    .querySelectorAll("div .ayah")
    .forEach((ayah, i) => {
...
function reloadThisAyahBackToAR(elem) {
  onceTranslatedAyahs.forEach((onceTranslate) => {
    if (
      elem.parentNode.id === "ayah" + onceTranslate.currentAyahID &&
      onceTranslate.previousTranslatedAyahContent.includes("audio")
    ) {
...
function detectAyahToSaveInLastRead(ayah) {
  ...
  const lastReadChapterName =
    lastReadAyahSaver.parentNode.parentNode.parentNode.querySelector(
      "li strong",
    ).textContent;
...
function goToThis(el) {
  ...
  const nameFromElToMatch = el.parentNode.parentNode
    .querySelector("bdi + div")
    .textContent.split(":")[1]
    .trim();
  const numberFromElToMatch = el.parentNode.parentNode
    .querySelector("bdi + div")
    .textContent.split(":")[0];
...
function addTextareaValue(selectedAyahFromLocal, el, arrOfLocal) {
  ...
  el.addEventListener("blur", (e) => {
    const localValueChanged = arrOfLocal
      .map((bookmark) => {
...
function addNote(el) {
  ...
  el.parentNode.nextElementSibling.style.outline =
    "3px solid rgba(255,255,255,0.4)";
  el.parentNode.nextElementSibling.select();

  const ayahToMatchWithLocal =
    el.parentNode.parentNode.querySelector("bdi").textContent;
...
function removeThisBookmark(el) {
  ...
  const ayahToMatchWithLocal =
    el.parentNode.parentNode.querySelector("bdi").textContent;
  const ayahNameToMatchWith = el.parentNode.parentNode
    .querySelector("bdi + div")
    .textContent.split(":")[1]
    .trim();
  const ayahNumberToMatchWith = Number(
  ...
  checkingEl.querySelector("div").addEventListener("click", (e) => {
    if (e.target.tagName === "SPAN" && e.target.textContent === "Yes") {
      arrOfLocal.forEach((bookmark, i, thisArr) => {
        ...
        if (
          toMatchAyah.test(bookmark) &&
          toMatchAyahName.test(bookmark) &&
          toMatchAyahNumber.test(bookmark)
        ) {
...
autoScrollEl
  .querySelector('[data-tool="scroll"]')
    ...
    audio.addEventListener("click", (e) => {
      ...
      if (window.navigator.onLine) {
        ...
        audio.children[0].addEventListener("loadedmetadata", () => {
          ...
          const progressBy =
            (audio.parentNode.offsetWidth /
              (audio.children[0].duration * 1000)) *
            100;
          let countProgress = 0;
          //Extract percent progress
          let percentProgress =
            (progressBy * 100) / audio.parentNode.offsetWidth;
          int = setInterval(() => {
...
function highlightCurrentPlaying(idxOfThisAudio, thisAudio) {
  ...
  let int;
  const progressBy =
    (audio.parentNode.offsetWidth / (thisAudio.duration * 1000)) * 100;
  let countProgress = 0;
...
function detectCurrentChapterToListen(ayah) {
  ...
  elem.textContent =
    ayah.target.parentNode.parentNode.previousElementSibling.querySelector(
      "li > strong",
    ).textContent;
  if (restartToUpdateIdx) {
    currentAudIdx = parseInt(
      ayah.target.parentNode.children[0].id.split("ayah")[1],
    );
  }
}
...
function saveToLocal() {
  ...
  settings.floatingWindowPosition =
    localStorage.getItem("lastPositionOfFloatingWindow") ??
    lastPositionOfFloatingWindow;

  settings.lastReadDetails =
    localStorage.getItem("lastReadDetails") ?? lastReadDetails;
